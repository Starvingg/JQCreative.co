...
const sectionsData = [
  {
    title: "JQ Creative",
    content: ["A Modern Software Development Agency."],
  },
  {
...
const Figure8Particles = () => {
  ...
  const particlePositions = useMemo(
    () =>
      new Array(PARAMS.PARTICLE_COUNT).fill(0).map(() => new THREE.Vector3()),
    []
  );
  ...
  return (
    <instancedMesh
      ref={instancedMesh}
      args={[null, null, PARAMS.PARTICLE_COUNT]}
    >
...
const Scene = ({ scroll }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (timeSinceInteraction > 3 || scrollFactor > 0) {
      const resetFactor = Math.max(
        timeSinceInteraction > 3 ? 0.05 : 0,
        scrollFactor * 0.1
      );
    ...
    <div className="absolute inset-0 flex items-center justify-end p-6 pointer-events-none">
      <div className="text-white text-right">
        ...
        <p className="text-xs opacity-70 max-w-xs">
          ...
          wheels. We have to be patient to be victorious.
          <br />
          <br />
          Our mistake in all of our thinking is that we each believe ourselves
          to be an independent entity; one self beside countless other selves.
          While in reality, we're all just small fractions of an infinite whole.
        </p>
      </div>
    </div>
  );
    ...
    <div className="relative bg-black text-white">
      <div className="h-screen sticky top-0 w-full">
        <Canvas style={{ width: "100%", height: "100%" }}>
          <PerspectiveCamera
            makeDefault
            position={[0, 0, PARAMS.INITIAL_CAMERA_DISTANCE]}
            fov={75}
          />
